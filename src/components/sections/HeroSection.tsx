...
export default function HeroSection() {
  ...
  const isTyping = currentText.length < currentFullText.length;

  useEffect(() => {
    let timer: NodeJS.Timeout;

    if (currentText.length < currentFullText.length) {
      // 한 글자당 약 140ms로 묵직하고 여유로운 타이핑
      timer = setTimeout(() => {
        setCurrentText(currentFullText.slice(0, currentText.length + 1));
      }, 140);
    } else {
      // 문구 완성 후 대기: 3번 문구는 약 4초, 1·2번 문구는 약 2초 유지 후 지우는 모션 없이 바로 다음 문장 타이핑 시작
      const pauseDuration = phraseIndex === 2 ? 4000 : 2000;
      timer = setTimeout(() => {
  ...
  }, [currentText, currentFullText, phraseIndex]);
